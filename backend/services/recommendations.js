...
async function generateMLRecommendations(studentId, burnoutAnalysis) {
  ...
  // ============================================
  // ✅ NEW: SMART WORKLOAD RESTRICTIONS NOTICE
  // ============================================
  if (score >= 60) {
    recommendations.push({
      title: '🚫 Workload Protection Active',
      priority: 'critical',
      category: 'self_care',
      ...
      actions: [
        '🛑 System blocks: All exams, all projects, and tasks requiring >5 hours',
        '✅ You can still add: Light tasks (<5 hours), low-effort assignments',
        '📉 Restrictions will lift automatically when your score drops below 60',
        '🆘 If you have urgent academic requirements, contact your proctor immediately',
        '💆 Use this enforced pause to focus on recovery and existing commitments',
        '📞 Visit VIT Counseling Center (AB2 Building) for mental health support'
      ]
    });
  } else if (score >= 40) {
    recommendations.push({
      title: '⚠️ Selective Workload Management Active',
      priority: 'high',
      ...
      actions: [
        '⚠️ System blocks: Tasks requiring >8 hours effort',
        '✅ Strategy: Break large tasks into chunks of 3-5 hours each',
        '📊 You will receive warnings before adding exams or major projects',
        '📉 Restrictions ease as your burnout score improves',
        '🎯 Focus on completing existing work before adding more',
        '📈 Monitor your dashboard regularly to track progress'
      ]
    });
  } else if (score >= 30) {
    recommendations.push({
      title: '💡 Smart Workload Monitoring',
      priority: 'medium',
      category: 'prevention',
      ...
      actions: [
        '✅ No restrictions currently active',
        '⚠️ You will receive warnings for high-stress tasks',
        '📝 Plan major tasks carefully to avoid clustering deadlines',
        '🔍 System is tracking your workload patterns',
        '💪 Maintain current balance to keep risk low'
      ]
    });
  }
  ...
  if (signals?.collision?.hasCollision) {
    ...
    recommendations.push({
      ...
      actions: [
        ...
        '🛑 Note: System may block new task additions until workload stabilizes'
      ]
    });
  }
  ...
  if (signals?.volatility?.hasVolatility) {
    ...
    recommendations.push({
      ...
      actions: [
        ...
        severity === 'high' ? '⚠️ Consider if this spike justifies requesting deadline extensions' : 'Monitor if spike continues into next week'
      ]
    });
  }
  ...
  if (signals?.recovery?.hasRecoveryDeficit) {
    ...
    recommendations.push({
      ...
      actions: [
        ...
        streak > 10 ? '🚨 URGENT: Your streak is dangerously long - seek counseling support' : 'System will remind you to take breaks'
      ]
    });
  }
  ...
  if (signals?.drift?.hasDrift) {
    ...
    recommendations.push({
      ...
      actions: [
        ...
        severity === 'severe' ? '🆘 URGENT: Book counseling session - burnout may be affecting cognition' : 'Track which subjects need most attention'
      ]
    });
  }
  ...
  if (signals?.gradeAnalysis?.hasLowGrades) {
    ...
    recommendations.push({
      ...
      actions: [
        ...
        avgGrade < 50 ? '🚨 CRITICAL: Meet with academic advisor immediately' : 'Track grade trends weekly to catch declines early'
      ]
    });
  }
  ...
  if (risk === 'medium' || risk === 'high') {
    recommendations.push({
      ...
      actions: [
        ...
        risk === 'high' ? '📞 Emergency support: Student Welfare Office at Tower Block' : 'Join campus wellness programs and stress management workshops'
      ]
    });
  }
  ...
  if (risk === 'low' && recommendations.length === 0) {
    recommendations.push({
      ...
      actions: [
        ...
        '💡 The system will alert you if your risk increases',
        '🎯 Current approach is working - maintain consistency'
      ]
    });
  }
  ...
  if (score >= 30) {
    recommendations.push({
      title: '🤖 How the Protection System Works',
      priority: 'low',
      category: 'system_info',
      description: 'Understanding how burnout prevention works helps you plan better.',
      actions: [
        `📊 Your current score: ${score}/100 (${risk.toUpperCase()} risk)`,
        ...
        '📉 Complete existing work and rest to lower your score',
        '🔄 System recalculates automatically after each task completion',
        '👨‍🏫 Proctors are notified if you hit red zone repeatedly'
      ]
    });
  }
